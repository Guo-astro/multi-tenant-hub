...
for i in $(aws s3 ls | awk '{print $3}' | grep -E "^cicdstack-*|^deploy-*|systemprovisioning*|tenantprovisioningpipeli-*|tenantprovisiong*|systemproviderinfrastack*|pipelinestack*|costanalyticspipeline*|curbucket*"); do
...
delete_stack_after_confirming "infraDeploymentRootStack"

delete_stack_after_confirming "ciCdStack"
...
while true; do
    ...
    log_groups=$(echo "$response" | jq -r '.logGroups[].logGroupName | select(. | test("*|^/aws/lambda/*|^API-*|^/aws/codebuild/*|^/aws/codelambda/serverless-saas-*"))')
